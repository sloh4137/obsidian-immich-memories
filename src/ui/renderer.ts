...
function parseCodeblockParams(source: string): {
	dateOverride?: string;
	...
	for (const line of trimmed.split('\n')) {
		...
		if (
			['timezone', 'tz', 'timezone', 'time_zone'].includes(key) ||
			key === 'timezone'
		)
...
function createExplicitErrorEl(
	...
	if (!settings.immichServerUrl) {
		tips.push(
			'Set server URL in Settings → Immich Memories (e.g. https://immich.example.com, no trailing /api).'
		);
	} else {
		tips.push(
			`Verify server URL is correct and reachable: ${serverUrl} should open Immich in a browser.`
		);
		tips.push(
			'Base URL should NOT include /api – the plugin appends /api/search/metadata itself.'
		);
	}
	if (!settings.immichApiKey) {
		tips.push(
			'Set API key in Settings → Immich Memories. Create it in Immich → Account Settings → API Keys.'
		);
	} else if (msg.includes('401') || msg.includes('403')) {
		tips.push(
			'Your API key may be invalid or lack search permission. Regenerate it in Immich and update settings.'
		);
	}
	if (msg.includes('404')) {
		tips.push(
			'Endpoint /api/search/metadata requires Immich v1.90+. Update Immich if you run an older version.'
		);
		tips.push(
			'If behind reverse proxy (nginx, Cloudflare), ensure POST /api/search/metadata is not blocked.'
		);
	}
	if (
		...
		tips.push(
			'Check network/DNS, VPN, firewall, and that the Immich server is running.'
		);
		tips.push(
			'Self-signed certificates may be rejected by Electron – add CA or use valid cert.'
		);
	}
	if (tips.length === 0) {
...
export function createImmichBlockProcessor(
	...
	return async function processor(
		...
		if (!client.isConfigured()) {
			...
			wrapper.createDiv({
				cls: 'immich-memories-error-title',
				text: 'Immich is not configured',
			});
			wrapper.createDiv({
				cls: 'immich-memories-error-body',
				text: `Server URL and/or API key missing. Current server URL: ${
					settings.immichServerUrl || '(empty)'
				}. Open Settings → Immich Memories to configure.`,
			});
			return;
		}
		...
		if (!fm && !overrides.dateOverride) {
			createMessageEl(
				el,
				`No frontmatter found for ${ctx.sourcePath} and no date override in codeblock. Add "${settings.dateField}: YYYY-MM-DD" to frontmatter.`
			);
			return;
		}
		...
		if (!timeZoneStr && fm) {
			timeZoneStr =
				extractFrontmatterValue(fm, settings.timezoneField) || 'UTC';
		}
		...
		const loadingEl = createMessageEl(
			el,
			`Loading Immich memories for ${dateStr} (${timeZoneStr})...`,
			'immich-memories-loading'
		);
		...
		if (photos.length === 0) {
			createMessageEl(
				el,
				`No photos found in Immich for ${dateStr} in timezone ${timeZoneStr}. The connection succeeded, but no assets matched that day.`
			);
			return;
		}
		...
		const previewSection = container.createDiv({
			cls: 'immich-memories-preview-section',
		});
		previewSection.createDiv({
			cls: 'immich-memories-preview-label',
			text: `Memories from ${dateStr}`,
		});
		...
		const details = container.createEl('details', {
			cls: 'immich-memories-details',
		});
		const summary = details.createEl('summary', {
			cls: 'immich-memories-summary',
		});
